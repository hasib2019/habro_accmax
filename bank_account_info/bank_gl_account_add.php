...
if (isset($_POST['subBtn'])) {
  ...
  if ($conn->affected_rows == 1) {
    ...
    $mess = "Failled!";
    header("refresh:2;bank_acc_info.php?id=$idno");
  }
  // update 
  $conn->query("UPDATE `bank_acc_info` SET `gl_acc_code`='$acc_code' WHERE acc_name='$acc_head'");
...
<main class="app-content">
  <div class="app-title">
    <div>
      <h1><i class="fa fa-dashboard"></i> Bank GL A/C </h1>
    </div>
    ...
    <div class="col-md-12">
      ...
      <form action="" method="post">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-6">
            <select name="acc_head" id="" class="form-control" required>
              <?php
              $selectQuery = 'SELECT * FROM `bank_acc_info`';
              $selectQueryResult =  $conn->query($selectQuery);
              if ($selectQueryResult->num_rows) {
                while ($row2 = $selectQueryResult->fetch_assoc()) {
                  echo '<option value="' . $row2['acc_name'] . '">'  . $row2['acc_name'] . '</option>';
                }
              }
              ?>
            </select>
        ...
        <!-- <div class="form-group row">
          ...
          <div class="col-sm-6"> -->
        <input type="text" class="form-control" id="" value="2" name="acc_type" hidden>

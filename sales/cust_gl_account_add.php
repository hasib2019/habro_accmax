...
if (isset($_POST['subBtn'])) {
  ...
  if ($conn->affected_rows == 1) {
    ...
    $mess = "Failled!";
    header("refresh:2;cust_gl_account_add.php?id=$idno");
  }
}
//select query start
if (isset($_GET['id'])) {
...
<main class="app-content">
  <div class="app-title">
    <div>
      <h1><i class="fa fa-dashboard"></i> Customer GL A/C </h1>
    </div>
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-6">
            <select name="acc_head" id="acc_head" class="form-control" onchange="gl_account_check_availability()" required>
              ...
              <?php
              $selectQuery = 'SELECT * FROM `cust_info`';
              $selectQueryResult =  $conn->query($selectQuery);
              if ($selectQueryResult->num_rows) {
                while ($row2 = $selectQueryResult->fetch_assoc()) {
                  echo '<option value="' . $row2['cust_name'] . '">'  . $row2['cust_name'] . '</option>';
                }
              }
              ?>
            </select>

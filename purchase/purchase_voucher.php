...
if (isset($_POST["submit"])) {
    ...
    $insertQuerydr = "INSERT INTO `tran_details` (`tran_no`,`office_code`,`year_code`,`batch_no`, `tran_date`, `gl_acc_code`,`tran_mode`,`vaucher_type`, `description`, `dr_amt_loc`,`ss_creator`,`ss_creator_on`,`ss_org_no`) VALUES (NULL,'$office_code','$year_code','$batch_no','$tran_date','$by_account','$tran_mode','$vaucher_typedr','$particulardr','$draccount','$ss_creator',now(),'$ss_org_no')";
    $conn->query($insertQuerydr);
    if ($conn->affected_rows == 1) {
        $massage_dr = "dr Save Successfully!!";
    }
    $insertQuerycr = "INSERT INTO `tran_details` (`tran_no`,`office_code`,`year_code`,`batch_no`, `tran_date`, `gl_acc_code`,`tran_mode`,`vaucher_type`, `description`, `cr_amt_loc`,`ss_creator`,`ss_creator_on`,`ss_org_no`) VALUES (NULL,'$office_code','$year_code','$batch_no','$tran_date','$to_account','$tran_mode','$vaucher_typecr','$particularcr','$craccount','$ss_creator',now(),'$ss_org_no')";
    $conn->query($insertQuerycr);
    if ($conn->affected_rows == 1) {
        $massage_cr = "cr Save Successfully!!";
    }
    $insertQuerydis = "INSERT INTO `tran_details` (`tran_no`,`office_code`,`year_code`,`batch_no`, `tran_date`, `gl_acc_code`,`tran_mode`,`vaucher_type`,`description`, `cr_amt_loc`,`ss_creator`,`ss_creator_on`,`ss_org_no`) VALUES (NULL,'$office_code','$year_code','$batch_no','$tran_date','$discount_id','$tran_mode_discount','$vaucher_typecr','$discount','$discount_amount','$ss_creator',now(),'$ss_org_no')";
    if ($discount_amount != 0) {
        $conn->query($insertQuerydis);
    } else {
        echo  $conn->error;
    }
    $insertQueryvat = "INSERT INTO `tran_details` (`tran_no`,`office_code`,`year_code`,`batch_no`, `tran_date`, `gl_acc_code`,`tran_mode`,`vaucher_type`,`description`, `dr_amt_loc`,`ss_creator`,`ss_creator_on`,`ss_org_no`) VALUES (NULL,'$office_code','$year_code','$batch_no','$tran_date','$vat_id','$tran_mode_vat','$vaucher_typedr','$vat','$vat_amount','$ss_creator',now(),'$ss_org_no')";
    if ($vat_amount != 0) {
        $conn->query($insertQueryvat);
    } else {
        echo  $conn->error;
    }
    $insertQuerytax = "INSERT INTO `tran_details` (`tran_no`,`office_code`,`year_code`,`batch_no`, `tran_date`, `gl_acc_code`,`tran_mode`,`vaucher_type`,`description`, `dr_amt_loc`,`ss_creator`,`ss_creator_on`,`ss_org_no`) VALUES (NULL,'$office_code','$year_code','$batch_no','$tran_date','$tax_id','$tran_mode_tax','$vaucher_typedr','$tax','$tax_amount','$ss_creator',now(),'$ss_org_no')";
    if ($tax_amount != 0) {
    ...
    for ($count = 0; $count < count($_POST["item_no"]); $count++) {
        ...
        header('refresh:1;../purchase/purchase_voucher.php');
    }
}
            ...
            <tfoot>
                ...
                <tr>
                    ...
                    <td><select class="form-control grand" name="to_account" required>
                            ...
                            <?php
                            $selectQuery = "SELECT * FROM `gl_acc_code` where postable_acc= 'Y' AND acc_type=1 OR acc_type=6 ORDER by acc_code";
                            $selectQueryResult =  $conn->query($selectQuery);
                            if ($selectQueryResult->num_rows) {
                                while ($drrow = $selectQueryResult->fetch_assoc()) {
                                    echo '<option value="' . $drrow['acc_code'] . '">'  . $drrow['acc_head'] . '</option>';
